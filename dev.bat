...
if "%~1"=="bankincafe" set PROJECT_DIR=C:\Windsurf\bankincafe

REM 引数2がポート番号の場合
if not "%~2"=="" set PORT=%~2

REM 引数1がポート番号の場合（プロジェクト名でない場合）
if not "%~1"=="nenpi" if not "%~1"=="mimamori-calendar2" if not "%~1"=="hikaku" if not "%~1"=="voiceCast" if not "%~1"=="autocore" if not "%~1"=="bankincafe" if not "%~1"=="" (
