...
def process_direct_batch_from_csv():
    """CSVファイルから直接500件ずつバッチ処理"""
    
    print("=== split_work_items.csvから直接バッチ処理開始 ===")
    
    # CSVファイル読み込み
    df = pd.read_csv('batch_processing/split_work_items.csv')
    total_items = len(df)
    
    print(f"読み込み完了: {total_items}件のユニーク作業項目")
    ...
    with open(status_file, 'w', encoding='utf-8') as f:
        f.write('batch_num,status,processed_count,avg_confidence\n')
